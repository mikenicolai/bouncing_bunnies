...
assert(run("document.querySelector('#buildInfo').textContent.includes(BUILD_INFO.deployment)"));assert.equal(run('BUILD_INFO.build'),2026091302);assert.equal(run('BUILD_INFO.version'),'0.6.1');assert(Number.isFinite(Date.parse(run('BUILD_INFO.builtAt'))));
