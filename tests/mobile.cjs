...
assert(run("document.querySelector('#buildInfo').textContent.includes(BUILD_INFO.deployment)"));assert.equal(run('BUILD_INFO.build'),2026092605);assert.equal(run('BUILD_INFO.version'),'0.7.4');assert(Number.isFinite(Date.parse(run('BUILD_INFO.builtAt'))));
