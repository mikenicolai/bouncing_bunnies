...
for(const fps of [30,60,120]){
 ...
 for(let i=0;i<fps;i++)tick();assert.equal(run('lives'),2,'tutorial sink-through now drops into the void');
 ...
 land(1960,-30);for(let i=0;i<fps*4;i++)tick();assert.equal(run('lives'),2,'later blue drops into void');assert(run('!player.blueContact'));
