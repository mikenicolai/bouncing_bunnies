...
assert(run('firePlatforms[0].y-firePlatforms[1].y>=100'));
assert(run('firePlatforms[2].y-firePlatforms[3].y>=100'));
assert.equal(run('coins.length'),12);
assert.equal(run('enemies.length'),0);

// The high climbs need one extra Jump; the drops are wider but remain safe.
for(const fps of [30,60,120])for(let i=0;i<firePlatformsLength();i++){
  ...
  for(let f=0;f<fps*3;f++){
    if((i===0||i===2)&&f===Math.round(fps*.1))run('jump()');
...
assert.equal(run('checkpoint.x'),445);
place(760,470);run('update(1/60)');
assert.equal(run('lives'),2);assert.equal(run('player.x'),445);
assert.equal(run('player.y'),287);
